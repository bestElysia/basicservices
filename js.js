export default {
  async fetch(request, env) {
    ...
    if (path === '/api/all') {
      ...
      const baseTotal = 56965;
      const baseOnline = 22;
      const baseAverageDuration = 1;

      // 计算实时数据并乘以8然后加上基础数据
      const realtimeToday = logs.filter(l => l.time.slice(0, 10) === today).length * 9;
      const realtimeNewUsers = todayIPs.size * 8;
      const realtimeTotal = keys.length * 7;
      const realtimeOnline = seenIPs.size * 6;
...
<head>
  ...
  <style>
    ...
    .card-hover:hover { transform: translateY(-8px); box-shadow: 0 20px 25px -5px rgba(0,0,0,0.1), 0 10px 10px -5px rgba(0,0,0,0.04); transition: all 0.3s ease; }
...
<body class="bg-gradient-to-br from-purple-50 via-pink-50 to-indigo-100 min-h-screen antialiased">
  <div class="container mx-auto px-4 py-8 max-w-7xl">

    <!-- 标题 -->
    <div class="text-center mb-12 animate-fade-in">
      <h1 class="text-4xl sm:text-5xl md:text-6xl font-bold gradient-text mb-4">全站监控中心</h1>
      <p class="text-lg sm:text-xl text-gray-600">elysia.bestxuyi.us 及所有子域</p>
    </div>

    <!-- 统计卡片（玻璃拟态） -->
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 sm:gap-6 mb-12">
      <div class="glass rounded-2xl p-4 sm:p-6 text-center card-hover transition-all animate-slide-up" style="animation-delay: 0.1s;">
        <i class="ri-eye-line text-3xl sm:text-4xl text-indigo-600 mb-2 sm:mb-3"></i>
        <div class="text-3xl sm:text-4xl font-bold text-gray-800" id="today">-</div>
        <div class="text-sm sm:text-base text-gray-600">今日访问</div>
      </div>
      <div class="glass rounded-2xl p-4 sm:p-6 text-center card-hover transition-all animate-slide-up" style="animation-delay: 0.2s;">
        <i class="ri-global-line text-3xl sm:text-4xl text-purple-600 mb-2 sm:mb-3"></i>
        <div class="text-3xl sm:text-4xl font-bold text-gray-800" id="total">-</div>
        <div class="text-sm sm:text-base text-gray-600">历史总访问</div>
      </div>
      <div class="glass rounded-2xl p-4 sm:p-6 text-center card-hover transition-all animate-slide-up" style="animation-delay: 0.3s;">
        <i class="ri-user-voice-line text-3xl sm:text-4xl text-orange-600 mb-2 sm:mb-3"></i>
        <div class="text-3xl sm:text-4xl font-bold text-gray-800" id="online">-</div>
        <div class="text-sm sm:text-base text-gray-600">当前在线</div>
      </div>
      <div class="glass rounded-2xl p-4 sm:p-6 text-center card-hover transition-all animate-slide-up" style="animation-delay: 0.4s;">
        <i class="ri-timer-line text-3xl sm:text-4xl text-blue-600 mb-2 sm:mb-3"></i>
        <div class="text-3xl sm:text-4xl font-bold text-gray-800" id="averageDuration">-</div>
        <div class="text-sm sm:text-base text-gray-600">平均响应时间</div>
      </div>
    </div>

    <div class="grid lg:grid-cols-2 gap-6 sm:gap-8 mb-12">
      <!-- 国家分布 -->
      <div class="bg-white/80 backdrop-blur rounded-3xl shadow-xl p-6 sm:p-8 card-hover transition-all animate-fade-in">
        <h2 class="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 flex items-center"><i class="ri-earth-line mr-3 text-indigo-600"></i> 全球访客分布</h2>
        <canvas id="countryChart"></canvas>
      </div>
      <!-- 24小时趋势 -->
      <div class="bg-white/80 backdrop-blur rounded-3xl shadow-xl p-6 sm:p-8 card-hover transition-all animate-fade-in">
        <h2 class="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 flex items-center"><i class="ri-line-chart-line mr-3 text-purple-600"></i> 24小时访问趋势</h2>
        <canvas id="trendChart"></canvas>
      </div>
    </div>

    <!-- 实时日志 -->
    <div class="bg-white/80 backdrop-blur rounded-3xl shadow-xl p-6 sm:p-8">
      <div class="flex flex-col md:flex-row justify-between items-center mb-4 sm:mb-6 gap-4">
        <h2 class="text-xl sm:text-2xl font-bold flex items-center"><i class="ri-history-line mr-3 text-green-600"></i> 实时访问记录</h2>
      ...
      <div id="list" class="space-y-4 max-h-[60vh] overflow-y-auto pr-2"></div>
    </div>
  </div>
  ...
  <script>
    ...
    async function load() {
      ...
      document.getElementById('list').innerHTML = data.logs.map(l => `
        <div class="bg-gradient-to-r from-indigo-50 to-purple-50 p-4 sm:p-5 rounded-2xl border border-indigo-100 hover:shadow-md transition">
          <div class="flex justify-between items-start flex-wrap gap-3">
            <div>
              <strong class="text-indigo-700 text-base sm:text-lg">${l.ip}</strong>
              <span class="ml-2 sm:ml-3 bg-indigo-100 text-indigo-800 px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm">${l.country}</span>
              <span class="ml-1 sm:ml-2 bg-purple-100 text-purple-800 px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm">${l.domain}</span>
            </div>
            <div class="text-xs sm:text-sm text-gray-500">${l.time}</div>
          </div>
          <div class="mt-2 text-gray-700 font-medium text-sm sm:text-base">${l.path}</div>
          ...
          <button onclick="banIP('${l.ip}')" class="mt-3 px-3 sm:px-4 py-1 sm:py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition text-sm">封禁 IP</button>
        </div>
